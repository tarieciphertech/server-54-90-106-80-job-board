...
# Write each variable on its own line safely
sudo tee /etc/jobboard.env > /dev/null <<EOF
...
# Export only simple variables safely
export FLASK_ENV=production
export SECRET_KEY="${SECRET_KEY}"
export DATABASE_URL="postgresql://jobboard_user:${DB_PASSWORD}@localhost/jobboard_db"
...
sudo tee /etc/nginx/sites-available/jobboard > /dev/null <<'EOF'
server {
    listen 80;
...
sudo tee /etc/systemd/system/jobboard.service > /dev/null <<'EOF'
[Unit]
